...
1. Read the file and check it against the BioLearn9 design system (defined in `CLAUDE.md` and `theme.css`):
   - Must link `../theme.css` (not embed dark-mode `:root` overrides)
...
3. Rebuild or adapt the file so it:
   - Links `../theme.css` with no dark-mode overrides
   - Uses the correct Google Fonts link
